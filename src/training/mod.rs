...
use crate::training::constraints::{
	derive_challenge,
	enforce_linear_vjp,
...
fn enforce_vjp_block(
	block: &VJPBlockIR,
	forward_weights: &ArrayD<Fr>,
	expected_commitment: &[u8; 32],
) -> Result<Option<ArrayD<Fr>>, String> {
...
fn commit_state(
	params_before: &ArrayD<Fr>,
	params_after: &ArrayD<Fr>,
	randomness_challenge: Fr,
...
pub fn prove_training_step_ir(step: &TrainingStepIR) -> Result<TrainingStateCommitment, String> {
	...
	let optimizer_update = step
		.optimizer
		.updates
		.first()
		.ok_or("training step missing optimizer updates".to_string())?;
	...
	let digest = commit_state(&optimizer_update.w_t, &optimizer_update.w_next, step.randomness_challenge)
...
pub fn inference_as_training(
	input: ArrayD<Fr>,
	weights: ArrayD<Fr>,
	with_relu: bool,
) -> Result<TrainingStepIR, String> {
...
mod training_ir_tests {
	...
	fn build_linear_relu_step(
		...
		let update = SGDUpdateIR {
			...
			grad: grad_w.clone(),
			lr: Fr::from(lr_scale),
			w_next: apply_sgd(&weights, &grad_w, Fr::from(lr_scale)),
		};
		let mut step = TrainingStepIR {
			forward: ForwardIR {
				...
				activation,
				saved_for_backward: saved_for_backward.clone(),
				saved_commitment: saved_commitment,
				with_relu: true,
			},
			backward: BackwardIR {
				vjp_blocks: vec![VJPBlockIR {
